...
def bar_chart(xvalues: list, yvalues: list, ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '',
              percentage: bool = False, rotation: bool = False, tm: float = 0.05, ft: int = 6):
    ax = set_elements(ax=ax, title=title, xlabel=xlabel, ylabel=ylabel, percentage=percentage)
    set_locators(xvalues, ax=ax, rotation=rotation)
    ax.bar(xvalues, yvalues, edgecolor=cfg.LINE_COLOR, color=cfg.FILL_COLOR, tick_label=xvalues)
    for i in range(len(yvalues)):
        ax.text(i, yvalues[i] + tm, f'{yvalues[i]:.2f}', ha='center', fontproperties=FontProperties(size=ft))


def multiple_bar_chart(xvalues: list, yvalues: dict, ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '',
                       percentage: bool = False, tm: float = 0.05, ft: int = 6, location: str = 'best'):
    ax = set_elements(ax=ax, title=title, xlabel=xlabel, ylabel=ylabel, percentage=percentage)
    ngroups = len(xvalues)
    nseries = len(yvalues)
    ...
    for metric in yvalues:
        ...
        for k in range(len(values)):
            ax.text(pos_group[k], values[k] + tm, f'{values[k]:.2f}', ha='center', fontproperties=FontProperties(size=ft))
...
def plot_evaluation_results(labels: ndarray, trn_y, prd_trn, tst_y, prd_tst):
    ...
    multiple_bar_chart(['Train', 'Test'], evaluation, ax=axs[0], percentage=True, location='lower center')
    plot_confusion_matrix(cnf_mtx_tst, labels, ax=axs[1], title='Test')


def horizontal_bar_chart(elements: list, values: list, error: list = [], ax: plt.Axes = None,
...
def plot_confusion_matrix(cnf_matrix: ndarray, classes_names: ndarray, ax: plt.Axes = None,
                          normalize: bool = False, title=''):
    if ax is None:
        ax = plt.gca()
    if normalize:
...
def dummify(df, vars_to_dummify):
    other_vars = [c for c in df.columns if c not in vars_to_dummify]
